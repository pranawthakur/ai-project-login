...
## Session update (this pass): cardio_engine.py built + wired; split_engine.py multi-factor upgrade scoped, NOT started

### Context — three-item work order agreed with client this session
1. Split selection should weigh more than just days/week — activity level
   and current-vs-target weight (secondary, tiebreaker-only factors) on
   top of the existing days/training-age/goal decision tree.
2. Add a real cardio rule set (previously: `cardio` was just a day-token
   name with zero actual programming logic behind it).
3. Review the calorie/protein math and the food ingredient dataset it
   draws from; expand the dataset if thin.

Agreed approach for all three: **discuss and get explicit sign-off on the
rule set BEFORE writing code.** This pass built and shipped item #2 in
full. Item #1 (split engine) was scoped and discussed in detail but
**no code was written yet** — that's the next task. Item #3 not started.

---

### ✅ DONE THIS PASS — `cardio_engine.py` (item #2)

**New file**: `promptgen-backend/app/cardio_engine.py`. Deterministic,
pure-function rule engine, same philosophy/style as `split_engine.py` and
`programming_rules.py` — no LLM involvement, independently unit-testable.

Rule set implemented (all discussed and confirmed with client before
building):

1. **Weekly volume table** — sessions × minutes, keyed by
   (goal × training-age tier). Goals: fat_loss, muscle_gain (alias:
   bodybuilding), strength, general_fitness, athletic, recovery.
2. **Weight-gap modifier** (fat_loss only) — gap ≥10% of current
   bodyweight bumps volume up toward the ceiling; never applies to other
   goals, never reduces.
3. **Hard ceiling**: 4 sessions × 30 min/week, regardless of gap size — a
   bigger gap gets a longer *program duration*, never more weekly cardio.
4. **Modality gating**: beginners are LISS-only, full stop. HIIT unlocked
   only for intermediate/advanced + (fat_loss or athletic) goal, capped at
   1 HIIT session/week. muscle_gain/strength/recovery are always
   LISS-only regardless of training age. An impact-relevant injury
   disclosure (knee/hip/ankle — reuses `exercise_database.INJURY_KEYWORDS`
   + the same negation-aware matcher, not a second copy) disables HIIT
   entirely.
5. **Recovery-capacity override** (can only reduce, never increase):
   lifting days/week ≥6 → cardio capped at 2 sessions/week; ==5 → capped
   at 3; ≤4 → no extra cap.
6. **Placement priority**: (a) existing rest-day slots first
   (standalone), (b) existing cardio-designated day tokens (e.g.
   `cardio_core` from the Machine-Based split) filled directly — never
   double-stacked with a separate finisher, (c) remaining sessions attach
   as a post-lifting finisher, push/upper days first, **leg day last**
   unless the top fat_loss weight-gap tier is active (then leg day is
   eligible too). **Cardio never displaces a lifting day** — always
   additive or attached, never a trade.
7. **Duration-budget interaction**: a finisher's minutes count AGAINST
   that day's `programming_rules.session_duration_cap()` time budget —
   see `effective_session_minutes()`, which reduces the effective session
   length BEFORE `_compute_day_plan()` runs, so the lifting-exercise-count
   cap actually shrinks instead of the cardio landing as unbudgeted
   overflow. Standalone cardio days are NOT subject to
   `session_duration_cap()` (that function caps lifting-exercise count
   specifically).

**Wiring in `fitness_generator.py`**:
- `from . import cardio_engine` added to the import block.
- `build_user_prompt()`, step 7b (right after `weekly_template` is
  built): calls `cardio_engine.build_cardio_plan()` and stores the result
  on `profile["_cardio_plan"]` — same convention as the existing
  `_weekly_template`/`_vol` stash-on-profile pattern.
- `build_deterministic_workout_days()`: looks up each day's cardio
  placement by index (`cardio_by_day_index`). A rest day that got a
  standalone session is converted into a real cardio day (warmup + safety
  text, no lifting exercises) instead of staying an empty rest slot. A
  lifting day that got a finisher gets its effective session_minutes
  reduced before `_compute_day_plan()` runs (rule #7 above). Every
  cardio-bearing day gets an additive `"cardio": {...}` key — same
  pattern as the existing optional `_injury_safety_note`/`_validation`/
  `_progression_note` keys. **Nothing existing was renamed, removed, or
  changed in shape** — a caller that ignores the new `"cardio"` key sees
  byte-identical output to before this pass.
- New helper `_cardio_day_summary()` — frontend/prompt-facing shape
  (`{"mode", "minutes", "modality"}`), strips the internal `day_index`.

**Verified this pass**:
- `cardio_engine.py` unit-tested standalone across 6 scenarios: beginner
  fat_loss/no gap, advanced fat_loss/large gap/6-day (recovery cap +
  weight-gap modifier + HIIT all fire correctly together), advanced
  strength/5-day, beginner general_fitness with an existing `cardio_core`
  split day (correctly filled directly, not double-stacked), intermediate
  fat_loss with a disclosed knee injury (HIIT correctly disabled), and the
  negation-matcher edge case — "no knee issues" correctly does **not**
  trigger the injury gate (this exact bug class already bit
  `exercise_database.py` once before; confirmed the reused matcher avoids
  it here too).
- Full pipeline smoke test: stubbed `SUPABASE_URL`/`SUPABASE_KEY` env vars
  and installed missing sandbox deps (`pydantic-settings`, `supabase`,
  `python-dotenv`, `jinja2` — these were just absent from this sandbox,
  not a real project issue) to actually run
  `build_user_prompt()` → `build_deterministic_workout_days()`
  end-to-end for a 6-day/intermediate/fat_loss/large-weight-gap profile.
  Confirmed: cardio plan computed correctly, weekly_template's rest day
  (index 3) correctly converted to a standalone HIIT day, the push day
  that got a finisher correctly dropped from 5 to 2 exercises (duration
  budget actually enforced, not just labeled), all other days unaffected.
- `python3 -m py_compile` clean on both files.
- **Not yet tested**: against a live Supabase/Render deploy, or through
  an actual Gemini-backed `/result` request (only the deterministic
  Python path was exercised — that's the part this work touched, so
  that's the part that needed verification, but a real end-to-end HTTP
  request through `main.py` hasn't been run this pass).

---

### 🔜 NEXT UP — split_engine.py multi-factor upgrade (item #1) — SCOPED, NOT STARTED

Full design was discussed and is written up in the chat session (not yet
transcribed into a doc file — do that first if picking this up cold).
Summary of the agreed approach:

- **Keep the existing days/week × training-age × explicit-goal decision
  tree as the hard primary branch, unchanged.** The client-mandated hard
  rules (exactly 3 days → PPL, exactly 6 days → PPL, ≤2 days → full body,
  never Bro Split under 2yr training age, never 7 hard days) stay exactly
  as they are.
- **Activity level** (`_activity_modifier()` already exists in
  `split_engine.py` but is currently dead/unused — re-wire it) acts as a
  **tiebreaker within a day-count branch**, not a day-count shifter like
  the old legacy 24-row table used it: sedentary/light → prefer the
  lower-fatigue option in a tied branch; very_active/extreme → prefer the
  higher-volume/specialized option.
- **Weight-gap** (current vs target, same ≥10%-of-current-bodyweight
  threshold as `cardio_engine.py` for consistency) is a **lower-priority
  soft tiebreaker**: large gap toward fat_loss nudges toward a split with
  a built-in conditioning component IF the branch already offers one
  (e.g. `competition_prep`); large gap toward muscle_gain nudges toward
  the higher-volume tied option. **Never overrides an explicit stated
  goal** — tiebreaker only.
- **Cardio is explicitly OUT of scope for split selection** — agreed not
  to feed `cardio_engine.py`'s output back into split choice (would
  create a circular split→cardio→split dependency for marginal benefit).
  Cardio continues to just adapt to whatever split gets chosen, as built
  this pass.
- **Known constraint, flagged to client, not yet resolved**: most
  day-count branches in `_decide_master()` currently have only ONE
  possible output (3 and 6 days are hardwired to PPL, ≤2 is hardwired to
  full_body) — there's nothing to tiebreak there. Real tiebreak room only
  exists today at **4 days** (currently: strength→compound_strength,
  else→upper_lower — only 2 options) and **5 days**
  (advanced+priority→bro_split, else→hybrid). So in practice these new
  modifiers will only have visible effect at 4/5-day plans until/unless
  the 4-day branch gets a 3rd option added (recall the earlier-flagged
  gap: no advanced/bodybuilding-specific 4-day split exists — the old
  24-row table's row 4 "Beginner Bodybuilding" concept isn't replicated
  anywhere in the master-doc tree). **Client has not yet confirmed
  whether to expand the 4-day branch or accept modifiers having limited
  reach for now — ask before writing code.**

**Before starting**: re-confirm the above 4 bullets with the client in
one short message (don't just start coding off this handoff — the design
was agreed in chat, not finalized in writing, and the 4-day-branch
question is still open). Once confirmed, the implementation itself should
be small: a `_resolve_activity_tiebreak()` / `_resolve_weight_gap_tiebreak()`
pair of pure functions in `split_engine.py`, called only at the specific
branch points that currently have >1 option, following the exact same
docstring-cites-the-source-rule style already used throughout
`_decide_master()`.

---

### ⏳ NOT STARTED — food/calorie/protein review (item #3)

Investigated but no changes made. Findings, for whoever picks this up:

- The calorie/protein **math** in `fitness_generator._calculate_macros()`
  is solid — Mifflin-St Jeor BMR → TDEE → goal-based kcal target, with a
  genuinely well-reasoned realistic protein-kcal-cost fix already in place
  (`diet_engine.estimate_protein_kcal_per_g()` — protein costs 12-17
  kcal/g from real whole foods, not the naive flat 4 kcal/g). **This part
  does not need rework.**
- The actual gap is `app/food_database.py`'s `INGREDIENTS` dict — only
  **34 ingredients total**, all Indian-diet staples. Given
  `diet_engine.py` builds "3 distinct options per meal slot" from
  priority-ordered ingredient lists, 34 entries runs out of real variety
  fast, especially for vegan/allergy-restricted tiers (thinnest
  represented tier).
- Proposed expansion (discussed, not agreed in writing): grow to roughly
  90–120 ingredients, same schema (`kcal`, `protein_g`, `carb_g`,
  `fat_g`, `unit`, `unit_grams`, `max_grams`), weighted toward more
  protein-source variety per diet tier (vegan especially) and more
  carb/side options. **Get explicit sign-off on the target ingredient
  count and category breakdown before writing the data — this is a data-
  entry-heavy task, worth confirming scope first so it isn't redone.**

---

## Session update (previous pass): weekday picks now actually wired to the schedule
